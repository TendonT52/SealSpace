        ...
        <div className="relative w-full">
            ...
            {status === 'OK' && (
                    <div className="absolute z-50 w-full">
                        {renderSuggestions({ data, setValue, clearSuggestions, onAddressSelect })}
                    </div>
            )}
        </div>
    );
    ...
    return data.map((suggestion, index, array) => {
        ...
        return (
            <div
                ...
                className={`${index === array.length - 1 ? 'rounded-b-2xl' : ''} border ${index % 2 === 1 ? 'bg-ice' : 'bg-stone'} px-3 font-roboto text-base text-allports placeholder:text-jetstream hover:text-navy hover:drop-shadow-md  focus:border-transparent focus:outline-none`}
            >
